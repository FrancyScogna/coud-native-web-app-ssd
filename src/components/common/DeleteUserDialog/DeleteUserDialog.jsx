...
function DeleteUserDialog({ open, setOpen, userProfile, setUserProfile }) {
  ...
  const deleteFn = async () => {
    try {
      ...
      dispatch(
        showSnackbar({ message: "Hai eliminato l'utente dalla piattaforma." })
      );
      setOpen(false);
    } catch (error) {
      dispatch(
  ...
  return (
    <Dialog open={open} className='deleteuser-dialog'>
      ...
      <DialogContent className='dialog-content'>
        ...
        <Typography className='description'>
          {"Per eliminare l'utente, digita "}
          <strong>conferma</strong> e clicca su{' '}
          <strong style={{ color: 'red' }}>Elimina</strong>.
        </Typography>
        <TextField
          size='small'
          fullWidth
